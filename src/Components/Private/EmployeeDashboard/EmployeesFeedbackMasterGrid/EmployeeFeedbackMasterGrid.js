...
export const EmployeeFeedbackMasterGrid = () => {
  ...
  // ─── Fetch all feedback (now includes embedded callSummary) ─────────
  ...
  const fetchServiceReviews = async () => {
    try {
      const res = await axios.get(
        ...
        { params: { month: leaderMonth, year: leaderYear }, headers: authHeader() }
      );
      if (res.data.success) setServiceReviews(res.data.reviews || []);
    } catch (e) { console.error("fetchServiceReviews:", e); setServiceReviews([]); }
  };
  ...
  const fetchHRReviews = async () => {
    ...
    } catch (e) { console.error("fetchHRReviews:", e); setHrReviews([]); }
  };

  useEffect(() => { fetchAllData(); }, [UpdatePopUpShow]);
  useEffect(() => { fetchServiceReviews(); fetchHRReviews(); }, [leaderMonth, leaderYear]);

  // ─── Call stats computed from embedded callSummary (no separate API) ─
  const callStats = useMemo(() => {
    let totalAll = 0, answeredAll = 0;
    allFeedbacks.forEach(fb => {
      const cs = fb.callSummary;
      if (cs) {
        totalAll += cs.totalCalls || 0;
        answeredAll += cs.answeredCalls || 0;
      }
    });
  ...
  const stats = useMemo(() => {
    const result = { total: allFeedbacks.length, pending: 0, completed: 0, highRating: 0, avgRating: 0 };
    let totalRating = 0, ratingCount = 0;
    allFeedbacks.forEach((fb) => {
      const hasFeedback = fb.feedback && fb.feedback.rating;
      if (hasFeedback) {
        result.completed++;
        totalRating += fb.feedback.rating;
        ratingCount++;
        if (fb.feedback.rating >= 4) result.highRating++;
      } else {
        result.pending++;
      }
    });
  ...
  const customerLeaderMap = useMemo(() => {
    ...
    allFeedbacks.forEach((fb) => {
      ...
      (fb.allotTo || []).forEach((engineer) => {
        ...
        if (!map[engineer._id]) {
          map[engineer._id] = { name: engineer.name, totalRating: 0, jobCount: 0, highRatingCount: 0 };
        }
  ...
  const serviceReviewMap = useMemo(() => {
    ...
    serviceReviews.filter((r) => r.month === leaderMonth && r.year === leaderYear)
      .forEach((r) => { map[r.engineerId?.toString()] = parseFloat(r.avgRating) || 0; });
    return map;
  }, [serviceReviews, leaderMonth, leaderYear]);

  const hrReviewMap = useMemo(() => {
    const map = {};
    hrReviews.filter((r) => r.month === leaderMonth && r.year === leaderYear)
      .forEach((r) => { map[r.engineerId?.toString()] = parseFloat(r.avgRating) || 0; });
    return map;
  }, [hrReviews, leaderMonth, leaderYear]);

  // ─── Top 3 Combined Engineers ─────────────────────────────────────────
  const combinedTopEngineers = useMemo(() => {
    ...
    return [...allEngIds]
      .map((engId) => {
        ...
        const custAvg = custData && custData.jobCount > 0 ? parseFloat((custData.totalRating / custData.jobCount).toFixed(2)) : 0;
        ...
        const weightedScore = computeWeightedScore(combinedScore, custData?.jobCount || 1, globalAvg, 3);

        let name = custData?.name || "Unknown";
        if (name === "Unknown") {
          const sr = serviceReviews.find((r) => r.engineerId?.toString() === engId);
          const hr = hrReviews.find((r) => r.engineerId?.toString() === engId);
          name = sr?.engineerName || hr?.engineerName || "Unknown";
        }

        return {
          id: engId, name, jobCount: custData?.jobCount || 0, highRating: custData?.highRatingCount || 0,
          customerAvg: custAvg, serviceAvg, hrAvg,
          combinedScore: parseFloat(combinedScore.toFixed(2)),
          weightedScore: parseFloat(weightedScore.toFixed(2)),
        };
      })
      .filter((e) => e.name && e.name !== "Unknown")
      .sort((a, b) => b.weightedScore - a.weightedScore || b.combinedScore - a.combinedScore || b.jobCount - a.jobCount)
      .slice(0, 3);
  ...
  const getCallBadgeStyle = (answered, total) => {
    if (total === 0) return { bg: '#f8f9fa', color: '#adb5bd', border: '#dee2e6' };
    if (answered === 0) return { bg: '#f8d7da', color: '#721c24', border: '#dc3545' };
    if (answered === total) return { bg: '#d4edda', color: '#155724', border: '#28a745' };
  ...
  // Build tooltip text for call times
  const buildCallTimesTooltip = (callSummary) => {
    if (!callSummary || !callSummary.callTimes || callSummary.callTimes.length === 0) return "";
    const lines = callSummary.callTimes.map((ct, i) => {
      const dt = new Date(ct.dateTime);
      const dateStr = dt.toLocaleDateString('en-IN', { day: '2-digit', month: 'short' });
      const timeStr = dt.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true });
      const icon = ct.status === 'Answered' ? '✅' : '❌';
      return `  ${icon} ${i + 1}. ${dateStr} ${timeStr} — ${ct.status}`;
    });
    return `Call Attempts (${callSummary.totalCalls} total):\n${lines.join('\n')}`;
  };
          ...
          <div className="container-fluid page-body-wrapper">
            ...
            >
              <div className="content-wrapper ps-3 ps-md-0 pt-3">
                        ...
                        <div className="card-body">
                          <div className="row no-gutters align-items-center">
                            <div className="col mr-2">
                              <div className="text-xs font-weight-bold text-uppercase mb-1" style={{ color: s.color, fontSize: "0.75rem" }}>{s.label}</div>
                              {s.progress ? (
                                <div className="row no-gutters align-items-center">
                                  <div className="col-auto"><div className="h5 mb-0 mr-3 font-weight-bold text-gray-800">{s.val}</div></div>
                                  <div className="col"><div className="progress progress-sm mr-2"><div className="progress-bar bg-info" style={{ width: `${s.val * 20}%` }}></div></div></div>
                                </div>
                              ) : (
                                <div className="h5 mb-0 font-weight-bold text-gray-800">{s.val}</div>
                              )}
                            </div>
                            <div className="col-auto"><i className={`${s.icon} fa-2x text-gray-300`}></i></div>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                {/* ════════════════════════════════════════════
                    📞 CALL TRACKING STATS — Computed from embedded data
                ════════════════════════════════════════════ */}
                <div className="row px-2 pb-3">
                  {[
                    { label: "Total Call Attempts", val: callStats.totalAll, color: "#6f42c1", icon: "fas fa-phone" },
                    { label: "Answered Calls", val: callStats.answeredAll, color: "#28a745", icon: "fas fa-phone-flip" },
                    { label: "Not Answered Calls", val: callStats.notAnsweredAll, color: "#dc3545", icon: "fas fa-phone-slash" },
                    { label: "Answer Rate", val: `${callStats.answerRate}%`, color: "#20c997", icon: "fas fa-chart-pie", progress: true, progressVal: callStats.answerRate },
                  ].map((s) => (
                    <div key={s.label} className="col-12 col-md-3 mb-3">
                      <div className="card shadow h-100 py-2" style={{ borderLeft: `4px solid ${s.color}` }}>
                        <div className="card-body">
                          <div className="row no-gutters align-items-center">
                            <div className="col mr-2">
                              ...
                              {s.progress ? (
                                <div className="row no-gutters align-items-center">
                                  <div className="col-auto"><div className="h5 mb-0 mr-3 font-weight-bold text-gray-800">{s.val}</div></div>
                                  <div className="col"><div className="progress progress-sm mr-2"><div className="progress-bar" style={{ width: `${Math.min(s.progressVal, 100)}%`, backgroundColor: s.color }}></div></div></div>
                                </div>
                              ) : (
                                <div className="h5 mb-0 font-weight-bold text-gray-800">{s.val}</div>
                              )}
                        ...
                        <div>
                          <h6 className="m-0 font-weight-bold text-primary">
                            ...
                            <small className="text-muted ms-2" style={{ fontSize: '11px', fontWeight: 'normal' }}>
                              😊 Customer + 🔧 Service Manager + 👥 HR Review
                            </small>
                          </h6>
                        </div>
                        <div className="d-flex align-items-center gap-2">
                          <small className="text-muted fw-bold">Period:</small>
                          <select className="form-select form-select-sm" style={{ width: "auto" }} value={leaderMonth}
                            onChange={(e) => setLeaderMonth(Number(e.target.value))}>
                            {MONTHS.map((m, i) => (<option key={m} value={i}>{m}</option>))}
                          </select>
                          <select className="form-select form-select-sm" style={{ width: "auto" }} value={leaderYear}
                      ...
                      <div className="card-body pb-2">
                        {combinedTopEngineers.length === 0 ? (
                          <div className="text-center text-muted py-4" style={{ fontSize: "0.85rem" }}>
                            ...
                            No data for <strong>{MONTHS[leaderMonth]} {leaderYear}</strong>.
                          </div>
                        ) : (
                          <>
                              ...
                              {combinedTopEngineers.map((eng, index) => {
                                ...
                                return (
                                  <div key={eng.id} className="col-md-6 col-lg-4 mb-3">
                                    <div className="card shadow-sm h-100" style={{ borderLeft: `5px solid ${rank.color}`, borderRadius: '12px' }}>
                                      <div className="card-body py-3">
                                        <div className="d-flex align-items-center justify-content-between mb-3">
                                          <div className="d-flex align-items-center gap-2">
                                            <span style={{ fontSize: '2rem', lineHeight: 1 }}>{rank.icon}</span>
                                            <div>
                                              <div className="fw-bold text-dark" style={{ fontSize: '15px' }}>{eng.name}</div>
                                              <div className="d-flex align-items-center gap-1 mt-1">
                                                <span className={`badge ${expBadge.color}`} style={{ fontSize: '10px' }}>{expBadge.label}</span>
                                                <small className="text-muted" style={{ fontSize: '11px' }}>{rank.label}</small>
                                              </div>
                                            </div>
                                          </div>
                                        ...
                                        <div className="row text-center g-0 mb-2">
                                          {[
                                            ...
                                            <div key={src.label} className={`col-4 ${si < 2 ? 'border-end' : ''}`}>
                                              <div style={{ fontSize: '0.72rem', color: '#6b7280' }}>{src.icon} {src.label}</div>
                                              <div className="fw-bold" style={{ color: scoreBarColor(src.val), fontSize: "0.88rem" }}>
                                                {src.val > 0 ? `${src.val}/5` : <span style={{ color: '#d1d5db' }}>—</span>}
                                              </div>
                                              <div style={{ height: 3, background: '#e5e7eb', borderRadius: 2, margin: '3px 6px 0' }}>
                                                <div style={{ height: '100%', borderRadius: 2, width: `${(src.val / 5) * 100}%`, background: scoreBarColor(src.val), transition: 'width 0.6s ease' }} />
                                              </div>
                                            </div>
                                          ))}
                                        </div>
                                        <div className="d-flex align-items-center justify-content-between mt-2">
                                          <span style={{ fontSize: '0.78rem', color: '#6b7280', fontWeight: 600 }}>Combined</span>
                                          <span className="fw-bold" style={{ color: rank.color, fontSize: '1rem' }}>⭐ {eng.combinedScore} / 5</span>
                                        </div>
                                        <div className="mt-1 mb-2">
                                          <div className="progress" style={{ height: '6px', borderRadius: 4 }}>
                                            <div className="progress-bar" style={{ width: `${Math.min((eng.combinedScore / 5) * 100, 100)}%`, backgroundColor: rank.color, borderRadius: 4 }} />
                                          </div>
                                        </div>
                                        <div className="d-flex justify-content-between" style={{ fontSize: '0.75rem', color: '#6b7280' }}>
                                          <span><i className="fas fa-check-circle text-success me-1"></i>{eng.jobCount} jobs</span>
                                          <span><i className="fas fa-star text-warning me-1"></i>{eng.highRating} high</span>
                                          <span>W: {eng.weightedScore}</span>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                ...
                <div className="row px-2 py-2">
                  <div className="col-12">
                    <div className="d-flex align-items-center gap-3 flex-wrap">
                      ...
                      >
                        <i className="fa fa-clock me-2"></i>Pending Feedback ({pendingCount})
                      </button>
                      <button
                        ...
                        style={{ fontWeight: 'bold', minWidth: '180px' }}
                      >
                        <i className="fa fa-check-circle me-2"></i>All Feedback ({givenCount})
                      </button>
                      <button
                        ...
                        style={{ fontWeight: 'bold', minWidth: '180px' }}
                      >
                        <i className="fa fa-trophy me-2"></i>High Ratings Only ({highRatingCount})
                      </button>
                    </div>
                  </div>
                </div>
                    ...
                    <div className="d-flex align-items-center gap-2 flex-wrap">
                      ...
                      {[1, 2, 3, 4, 5].map((star) => (
                        <button
                          key={star}
                          onClick={() => {
                            if (selectedRating === star) { setSelectedRating(0); }
                            else { setSelectedRating(star); setShowFeedbackGiven(true); setShowHighRatingOnly(false); }
                          }}
                ...
                {/* ══════════════════════════════════════════════════════════
                    📋 MAIN TABLE — with Calls & Last Call columns
                ══════════════════════════════════════════════════════════ */}
                      ...
                      <table className="table table-striped table-class" id="table-id">
                        <thead>
                          <tr className="th_border">
                            ...
                            <th>Assigned to</th>
                            <th style={{ textAlign: 'center', minWidth: '90px' }}>📞 Calls</th>
                            <th style={{ minWidth: '140px' }}>Last Call</th>
                            <th>Rating</th>
                            <th>Status</th>
                            <th>Action</th>
                          </tr>
                        ...
                        <tbody className="broder my-4">
                          {paginatedFeedbacks.length > 0 ? (
                            paginatedFeedbacks.map((feedback, index) => {
                              ...
                              // Call data from embedded callSummary
                              const cs = feedback.callSummary;
                              const totalCalls = cs ? cs.totalCalls : 0;
                              const answeredCalls = cs ? cs.answeredCalls : 0;
                              const lastCallDT = cs ? cs.lastCallDateTime : null;
                              const lastCallStatus = cs ? cs.lastCallStatus : null;
                              const callBadgeStyle = getCallBadgeStyle(answeredCalls, totalCalls);
                              const callTooltip = buildCallTimesTooltip(cs);
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td style={{ whiteSpace: 'nowrap' }}>{formatDateTimeForDisplay(feedback.allotmentDate)}</td>
                                  <td style={{ whiteSpace: 'nowrap' }}>{formatDateTimeForDisplay(feedback.completionDate)}</td>
                                  <td>
                                    {feedback.allotTo?.map((person) => renderEngineerName(person, hasHighRating)) || 'N/A'}
                                  </td>
                                  ...
                                  <td style={{ textAlign: 'center' }}>
                                    {totalCalls > 0 ? (
                                      <span
                                        className="badge"
                                        style={{
                                          backgroundColor: callBadgeStyle.bg,
                                          color: callBadgeStyle.color,
                                          border: `1px solid ${callBadgeStyle.border}`,
                                          fontSize: "12px",
                                          fontWeight: "bold",
                                          cursor: "default",
                                          whiteSpace: "nowrap",
                                        ...
                                        title={callTooltip}
                                      >
                                        <i className={`fa ${answeredCalls > 0 ? 'fa-phone-flip' : 'fa-phone-slash'} me-1`}></i>
                                      ...
                                      >
                                        <i className="fa fa-phone-slash me-1"></i>0
                                      </span>
                                    )}
                                  </td>
                                    ...
                                    {lastCallDT ? (
                                      <div className="d-flex align-items-center">
                                        {lastCallStatus === "Answered" && (
                                          <i className="fa fa-phone-flip text-success me-1" style={{ fontSize: '11px' }}></i>
                                        )}
                                        {lastCallStatus === "Not Answered" && (
                                          <i className="fa fa-phone-slash text-danger me-1" style={{ fontSize: '11px' }}></i>
                                        )}
                                        {lastCallStatus && lastCallStatus !== "Answered" && lastCallStatus !== "Not Answered" && (
                                          <i className="fa fa-phone text-warning me-1" style={{ fontSize: '11px' }}></i>
                                        )}
                                        <span>{formatDateTimeForDisplay(lastCallDT)}</span>
                                        {lastCallStatus && (
                                          <span
                                            className="badge ms-1"
                                            style={{
                                              fontSize: '9px',
                                              backgroundColor: lastCallStatus === 'Answered' ? '#d4edda' : lastCallStatus === 'Not Answered' ? '#f8d7da' : '#fff3cd',
                                              color: lastCallStatus === 'Answered' ? '#155724' : lastCallStatus === 'Not Answered' ? '#721c24' : '#856404',
                                              border: `1px solid ${lastCallStatus === 'Answered' ? '#28a745' : lastCallStatus === 'Not Answered' ? '#dc3545' : '#ffc107'}`,
                                            }}
                                          >
                                            {lastCallStatus}
                                          </span>
                                  ...
                                  <td>
                                    <span className="badge" style={{ backgroundColor: cardType.borderColor, color: 'white' }}>
                                      {hasFeedback ? `Given (${cardType.type})` : 'Pending'}
                                    </span>
                                  </td>
                                  <td>
                                    <span
                                      ...
                                      title={hasFeedback ? "View/Update Feedback & Call History" : "Add Feedback & Log Call"}
                                    >
                            ...
                            <tr>
                              <td colSpan="13" className="text-center py-3">
                                {loading ? 'Loading...' :
                                  selectedRating > 0 ? `No feedback with ${selectedRating} star${selectedRating > 1 ? 's' : ''}.`
                  ...
                  <div className="pagination-container text-center my-3">
                    ...
                    {pageButtons.map((page) => (
                      <button key={page} onClick={() => setCurrentPage(page)}
                        className={`btn btn-sm me-1 ${currentPage === page ? "btn-primary" : "btn-dark"}`}>{page}</button>
                    ))}
