...
const UpdateProductPopUp = ({ handleUpdate, selectedProduct, categories = [] }) => {
  ...
  useEffect(() => {
    if (product?.productCategory && !allCategories.includes(product.productCategory)) {
      setAllCategories(prev => {
        if (!prev.includes(product.productCategory)) return [...prev, product.productCategory];
        return prev;
      });
    }
  ...
  useEffect(() => {
    if (product?.brandName && !allBrands.includes(product.brandName)) {
      setAllBrands(prev => {
        if (!prev.includes(product.brandName)) return [...prev, product.brandName];
        return prev;
      });
    }
  ...
  const handleProductUpdate = async (e) => {
    ...
    if (!product.productName || !product.baseUOM) { toast.error("Product Name and Base UOM are required fields"); return; }
    ...
    if (parseFloat(product.mrp) < parseFloat(product.salesPrice)) { toast.error("MRP must be greater than or equal to sales price"); return; }
    if (parseFloat(product.salesPrice) < parseFloat(product.minSalesPrice)) { toast.error("Sales price must be greater than or equal to minimum sales price"); return; }
    if (product.discountType !== "Zero Discount" && (!product.discountValue || isNaN(product.discountValue))) { toast.error("Please enter a valid discount value"); return; }
  ...
  const handleAddNewCategory = () => {
    if (newCategory.trim()) {
      ...
      setAllCategories([...allCategories, newCategory.trim()]);
      setProduct(prev => ({ ...prev, productCategory: newCategory.trim() }));
      setNewCategory(""); setShowAddCategory(false);
      toast.success("New category added successfully");
    } else { toast.error("Please enter a category name"); }
  };

  const handleAddNewBrand = () => {
    if (newBrand.trim()) {
      if (allBrands.includes(newBrand.trim())) { toast.error("Brand already exists"); return; }
      setAllBrands([...allBrands, newBrand.trim()]);
                ...
                <div className="row modal_body_height" style={{ maxHeight: "70vh", overflowY: "auto" }}>
                  ...
                  <div className="row mt-3">
                    <div className="col-md-6 col-12">
                      ...
                      <div className="input-group">
                        <input type="text" className="form-control rounded-0" id="brandName" name="brandName"
                          list="brandOptionsUpdate" value={product.brandName || ""} onChange={handleChange}
                      ...
                      {product.brandName && (
                        <small className="text-muted mt-1 d-block">
                          <i className="fa fa-info-circle me-1"></i>Currently saved: <strong>{product.brandName}</strong>
                        </small>
                      )}
                    </div>
                    <div className="col-md-6 col-12 mt-3 mt-md-0">
                      ...
                      <input type="text" className="form-control rounded-0" id="productName" maxLength={100}
                        name="productName" value={product.productName || ""} onChange={handleChange}
                        placeholder="Update Product Name...." required />
                    </div>
                  </div>
                  ...
                  <div className="row mt-3">
                    <div className="col-12">
                      ...
                      <div className="input-group">
                        <select className="form-select rounded-0" id="productCategory" name="productCategory"
                          value={product.productCategory || ""} onChange={handleChange} disabled={categoriesLoading}>
                      ...
                      {product.productCategory && (
                        <small className="text-muted mt-1 d-block">
                          <i className="fa fa-info-circle me-1"></i>Currently saved: <strong>{product.productCategory}</strong>
                        </small>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="row mt-3">
                    <div className="col-12">
                      ...
                      <select className="form-select rounded-0" id="category" name="category" value={product.category || ""} onChange={handleChange} required>
                        ...
                        {categoryOptions.map((option, index) => (<option key={index} value={option.value}>{option.label}</option>))}
                      </select>
                    </div>
                  </div>
                  ...
                  <div className="row mt-3">
                    <div className="col-12 col-md-6">
                      ...
                      <div className="input-group">
                        <input type="text" className="form-control rounded-0" id="currentStockQty" name="currentStockQty"
                          value={product.currentStockQty ?? ""} onChange={handleChange} placeholder="Enter current stock quantity" />
                        {product.baseUOM && <span className="input-group-text">{product.baseUOM}</span>}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="col-12 mt-3">
                    <div className="row border bg-gray mx-auto p-3">
                      ...
                      <div className="col-12 col-lg-4 mt-2"><div className="mb-3"><label htmlFor="mrp" className="form-label label_text">MRP</label><input type="text" className="form-control rounded-0" id="mrp" name="mrp" onChange={handleChange} value={product.mrp || ""} /></div></div>
                      <div className="col-12 col-lg-4 mt-2"><div className="mb-3"><label htmlFor="salesPrice" className="form-label label_text">Sales Price</label><input type="text" className="form-control rounded-0" id="salesPrice" name="salesPrice" onChange={handleChange} value={product.salesPrice || ""} /></div></div>
                      <div className="col-12 col-lg-4 mt-2"><div className="mb-3"><label htmlFor="purchasePrice" className="form-label label_text">Purchase Price</label><input type="text" className="form-control rounded-0" id="purchasePrice" name="purchasePrice" onChange={handleChange} value={product.purchasePrice || ""} /></div></div>
                      <div className="col-12 col-lg-4 mt-2"><div className="mb-3"><label htmlFor="minSalesPrice" className="form-label label_text">Min. Sales Price</label><input type="text" className="form-control rounded-0" id="minSalesPrice" name="minSalesPrice" onChange={handleChange} value={product.minSalesPrice || ""} /></div></div>
                      <div className="col-12 col-lg-4 mt-2"><div className="mb-3"><label htmlFor="minQtyLevel" className="form-label label_text">Min. Qty Level</label><input type="text" className="form-control rounded-0" id="minQtyLevel" name="minQtyLevel" onChange={handleChange} value={product.minQtyLevel || ""} /></div></div>
                      <div className="col-12 col-lg-4 mt-2"><div className="mb-3"><label htmlFor="discountType" className="form-label label_text">Discount Type</label><select className="form-select rounded-0" id="discountType" name="discountType" value={product.discountType || "Zero Discount"} onChange={handleChange}><option value="Zero Discount">Zero Discount</option><option value="In percentage">In Percentage</option><option value="In Value">In Value</option></select></div></div>
                      {product.discountType !== "Zero Discount" && (
                        <div className="col-12 col-lg-4 mt-2"><div className="mb-3"><label htmlFor="discountValue" className="form-label label_text">Discount Value</label><input type="text" className="form-control rounded-0" id="discountValue" name="discountValue" onChange={handleChange} value={product.discountValue || ""} placeholder={product.discountType === "In percentage" ? "Enter percentage" : "Enter value"} /></div></div>
                      )}

                      <div className="col-12 mt-3">
                        <h6 className="mb-3">Tax Details</h6>
                        <div className="col-12 mb-3">
                          <div className="form-check form-check-inline"><input className="form-check-input" type="radio" name="taxType" id="upTaxNone" value="none" checked={(product.taxType || "none") === "none"} onChange={handleChange} /><label className="form-check-label" htmlFor="upTaxNone">No Tax</label></div>
                          <div className="form-check form-check-inline"><input className="form-check-input" type="radio" name="taxType" id="upTaxGST" value="gst" checked={product.taxType === "gst"} onChange={handleChange} /><label className="form-check-label" htmlFor="upTaxGST">Add GST Rate</label></div>
                          <div className="form-check form-check-inline"><input className="form-check-input" type="radio" name="taxType" id="upTaxCESS" value="cess" checked={product.taxType === "cess"} onChange={handleChange} /><label className="form-check-label" htmlFor="upTaxCESS">Add CESS % / Amount</label></div>
                        </div>
                        {product.taxType === "gst" && (
                          <div className="row">
                            <div className="col-12 col-lg-6 mt-2"><div className="mb-3"><label htmlFor="upGstRate" className="form-label label_text">GST Rate</label><div className="input-group"><input type="text" className="form-control rounded-0" id="upGstRate" name="gstRate" value={product.gstRate || ""} onChange={handleChange} placeholder="Enter GST Rate" /><span className="input-group-text">%</span></div></div></div>
                            <div className="col-12 col-lg-6 mt-2"><div className="mb-3"><label htmlFor="upGstEffectiveDate" className="form-label label_text">GST Effective Date</label><input type="date" className="form-control rounded-0" id="upGstEffectiveDate" name="gstEffectiveDate" value={product.gstEffectiveDate ? product.gstEffectiveDate.substring(0, 10) : ""} onChange={handleChange} /></div></div>
                          </div>
                        )}
                        {product.taxType === "cess" && (
                          <div className="row">
                            <div className="col-12 col-lg-6 mt-2"><div className="mb-3"><label htmlFor="upCessPercentage" className="form-label label_text">CESS %</label><div className="input-group"><input type="text" className="form-control rounded-0" id="upCessPercentage" name="cessPercentage" value={product.cessPercentage || ""} onChange={handleChange} placeholder="Enter CESS Percentage" /><span className="input-group-text">%</span></div></div></div>
                            <div className="col-12 col-lg-6 mt-2"><div className="mb-3"><label htmlFor="upCessAmount" className="form-label label_text">CESS Amount</label><div className="input-group"><span className="input-group-text">₹</span><input type="text" className="form-control rounded-0" id="upCessAmount" name="cessAmount" value={product.cessAmount || ""} onChange={handleChange} placeholder="Enter CESS Amount" /></div></div></div>
                          </div>
                        )}
                      </div>
                    </div>
      ...
      {showAddCategory && (
        <div className="modal fade show" style={{ display: "block", backgroundColor: "#00000050", position: "absolute", zIndex: 9999, width: "100%" }}>
          <div className="modal-dialog modal-sm"><div className="modal-content">
            <div className="modal-header"><h5 className="modal-title">Add New Category</h5><button type="button" className="btn-close" onClick={() => setShowAddCategory(false)}></button></div>
            <div className="modal-body"><input type="text" className="form-control" value={newCategory} onChange={(e) => setNewCategory(e.target.value)} placeholder="Enter new category" /></div>
            <div className="modal-footer"><button type="button" className="btn btn-secondary" onClick={() => setShowAddCategory(false)}>Cancel</button><button type="button" className="btn btn-primary" onClick={handleAddNewCategory}>Add</button></div>
          </div></div>
        </div>
      )}
